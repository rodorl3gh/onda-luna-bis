...
const TYPES = [
  { value: "ENTRADA", label: "Entrada", icon: "M7 16V4m0 0L3 8m4-4l4 4" },
  { value: "SALIDA", label: "Salida", icon: "M17 16V4m0 0l-4 4m4-4l-4-4" },
  { value: "AJUSTE", label: "Ajuste", icon: "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" },
];

export default function MovementDialog({ open, onClose, materiaPrima, onSave }: MovementDialogProps) {
  ...
  if (!open || !materiaPrima) return null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (type === "SALIDA" && qty > materiaPrima.stock) {
      setError(`Stock insuficiente. Disponible: ${materiaPrima.stock}`);
      return;
    }
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-[var(--admin-bg-secondary)] border border-[var(--admin-border)] rounded-2xl p-6 w-full max-w-md shadow-2xl animate-scale-in">
        ...
        <div className="mb-4 p-3 rounded-xl bg-[var(--admin-bg-tertiary)] border border-[var(--admin-border)] flex items-center justify-between">
          ...
          <span className="text-sm font-bold text-[var(--admin-text)] font-mono">{materiaPrima.stock}</span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={saving}
              className="flex-1 py-2.5 rounded-xl bg-[var(--admin-accent)] text-white text-sm font-medium hover:bg-[var(--admin-accent-hover)] disabled:opacity-40 transition-colors"
            >
